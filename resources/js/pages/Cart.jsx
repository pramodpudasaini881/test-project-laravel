                    ...
                    <div className="xs:w-[95vw] sm:[90vw] md:w-5/6 mx-auto text-center sm:flex justify-between">
                        <div className="flex flex-col xs:text-xs md:text-sm p-1 sm:w-1/2 border-1">
                            {products.map((product) => {
                                const item = cartItems.find(e => e.id === product.id) || { count: 1 };
                                return (
                                    <div
                                        key={product.id}
                                        className='border-b-1 flex items-start relative xs:h-[110px] sm:h-[120px] py-4'>
                                        <p className="xs:text-xs sm:text-sm cursor-pointer hover:scale-110 absolute xs:top-0 xs:right-0" onClick={() => toggleItemCart(product.id)}>
                                            ❌
                                        </p>
                                        <Link to={`/product-details/${product.id}`} className='xs:w-1/3 xs:h-2/3 h-full m-auto flex justify-center items-center  sm:mr-2 md:mr-0'>
                                        ...
                                        <div className='xs:w-2/3 flex flex-col justify-start items-start gap-2 text-sm font-medium'>
                                            ...
                                            <p className="xs:text-xs md:text-sm gap-2 font-medium text-gray-500">
                                                Qty:
                                                <button
                                                    onClick={() => decCount(product.id, product.sellPrice)}
                                                ...
                                                <button
                                                    onClick={() => incCount(product.id, product.sellPrice)}
